...
CLI 경로일 때는 `claude.usage` 에 호출 수·실패·평균 지연·동시 실행 상한이 함께
실려 나옵니다. 대시보드 한 번 여는 데 브리핑·정세·트럼프 요약이 동시에 붙을 수
있어서 동시 실행을 막아 두었고, 상한에 걸린 요청은 줄을 서는 대신 로컬 엔진으로
내려갑니다 (`rejected` 로 집계).
...
| `MUJIN_CLAUDE_CLI_CONCURRENCY` | `2` | CLI 동시 실행 상한. 한 호출이 Node 프로세스 하나이고 모두 같은 구독 한도를 쓴다 |
...
| 키 | 동작 |
|---|---|
| `Ctrl + K` | 명령 팔레트 — 화면 이동, 뉴스/프리즘/최저가 검색, 그리고 수집된 기사 실시간 검색 (Enter로 원문 열기) |
| `Alt + A` | 야무진 비서 (AI 도크) |
